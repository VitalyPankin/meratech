export default {
  'about': {
    'lab': {
      'title': 'Научные исследования',
      'body': 'Наши научно-исследовательские лаборатории составляют ядро отдела разработок и контроля компании, главная из которых располагается во Франции, где уже более 40 лет занимается разработкой гигиенических продуктов.<br><br>В результате многолетней исследовательской работы с профильными научными институтами, химическими и биологическими кафедрами крупнейших университетов Европы, мы получили более 100 патентов на химические продукты и создали уникальный комплекс гигиенических продуктов, который отвечает любым требованиям рынка и способен реализовать самую сложную гигиеническую программу для наших клиентов.',
    },
    'production': {
      'title': 'Инновационное производство',
      'body': '<p>Наши производственные комплексы, сертифицированные по международным стандартам ISO 9001 и ISO 14001, располагаются в Европе и в России. Благодаря такому распределению производственных мощностей мы можем проводить гибкую, экономически выгодную для наших клиентов ценовую политику, а также соблюдать требования и нормы Европейского Экологического законодательства. </p> <p>В процессе производства мы используем уникальное оборудование и несколько фаз контроля качества. Каждая партия продукта проходит процесс тестирования на внутренней испытательной базе и проверяется на соответствие заявленным характеристикам и нормам безопасности в лабораторных условиях, и мы храним контрольные образцы от каждой из них. Мы используем только высокоэффективные передовые технологии и химическое сырье надежных европейских марок.</p>',
    },
    'ecology': {
      'title': 'Забота о будущем',
      'body_1': '<b>Мы признаем и понимаем</b> всю ответственность перед обществом, возлагаемую на нас как на компанию. Осознание того, что любой бизнес в отрыве от экологической доктрины несет в себе угрозу для окружающей среды, дает нам мощный стимул быть среди эконоваторов.<br/><br/><b>Мы думаем</b> о будущем нашей планеты, именно поэтому в основе нашей миссии лежит правило: при постоянном расширении бизнеса не навредить хрупкой экосистеме и людям.<br/><br/><b>Мы принципиально не делим</b> вопросы заботы об окружающей среде на важные и неважные. Как океан состоит из капель, экологическая миссия нашей компании складывается из многих компонентов: от экономии бумаги при документообороте до вторичной переработки продуктов с истекшим сроком годности.',
      'body_2': '<b>Мы заботимся</b> о пополнении водных ресурсов, отказываясь от традиционных способов розлива продукции, оптимизируем производственные циклы; при выборе поставщиков наш приоритет - это строгое соответствие производства и сырья современным требованиям экологического законодательства.<br/><br/><b>Мы</b> не только постоянно улучшаем внутренние бизнес-процессы, но и <b>несём экологические ценности</b> нашим клиентам. Среди решений, которые мы внедряем, всегда во главе: экономия воды и энергоресурсов, снижение нагрузки на сточные воды, безопасность труда для персонала.<br/><br/><b>Мы уверены</b> в том, что экологическая осознанность сегодня - это единственно возможный фундамент для лучшего будущего.'
    }
  },
  'index': {
    'menu': {
      'about': 'О компании',
      'press_center': 'Пресс-центр',
      'media': 'Медиа',
      'catalog': 'Каталог',
      'contacts': 'Контакты'
    },
    'equipment': {
      'title':'Производительность и надежность',
      'body':'Надежные спутники для поддержки комплексной гигиенической программы любого пищевого производства.',
    },
    'press_center': {
      'title':'Новости и <br/>научные публикации',
      'body':'На страницах этого раздела вы найдете справочную и аналитическую информацию, публикации и новостные материалы, которые помогут вам узнать больше о деятельности компании.'
    },
    'products': {
      'title':'Исключительная эффективность',
      'body':'Каталог самых современных профессиональных средств для предприятий любого сектора пищевой промышленности.'
    },
    'documents': {
      'title':'Медиа-материалы и документация',
      'body':'Сопровождающие и нормативные документы, каталоги и все материалы, которые расскажут<br/> вам о компании <span class="red">meratech<sup>&reg;</sup></span> и всех видах ее продукции.'
    },
    'video': {
      'title':'Узнайте о нас больше',
      'body':'Авторизация по логину и паролю представляет простой POST-запрос, в результате которого возвращается access token.',
      'more':'подробнее о'
    }
  },
  'common':{
    'all': 'Все',
    'articles': 'Статьи',
    'everything': 'Любые',
    'about_prep': 'О',
    'catalog': 'Каталог',
    'center': 'центр',
    'copyright': 'Copyright',
    'copyright_disclaimer': 'Все права защищены. Все упомянутые торговые марки являются собственностью  регистраторов и их использование должно быть согласовано.',
    'details': 'Подробнее',
    'documentation': 'Документация',
    'email': 'Адрес email',
    'equipment': 'Оборудование',
    'find_product': 'Найти продукт',
    'founded': 'найденных',
    'from': 'от',
    'get_catalog': 'Скачать каталог',
    'get_acticle': 'Скачать статью',
    'get_catalog_hash': {
      'poultry': {
        'title': 'Скачать каталог для птицефабрик',
        'body': 'Комплексная программа гигиены для птицефабрик и предприятий птицеперерабатывающей промышленности от профессионалов отрасли.',
      },
      'pigs': {
        'title': 'Скачать каталог для свиноводческих предприятий',
        'body': 'Программа гигиены для свиноводческих предприятий пищевой промышленности. Гигиена цехов выращивания и переработки мяса свиней.',
      },
      'cows': {
        'title': 'Скачать каталог для молочных ферм',
        'body': 'Каталог комплексных решений гигиены молочных ферм и животноводческих предприятий',
      },
      'meat': {
        'title': 'Скачать каталог для мясоперерабатывающей промышленности',
        'body': 'Универсальная программа гигиены для предприятий мясоперерабатывающей промышленности, обеспечивающая исключительный контроль гигиены на предприятии.',
      },
      'milk': {
        'title': 'Скачать каталог для молочной промышленности',
        'body': 'Комплексная программа гигиены для молокоперерабатывающих предприятий всех типов.',
      },
      'brewery': {
        'title': 'Скачать каталог для пивобезалкогольной промышленности',
        'body': 'Инновационная программа гигиены для предприятий пивобезалкогольного сектора пищевой промышленности.',
      },
      'laundry': {
        'title': 'Скачать каталог для прачечной',
        'body': 'Новейшая программа гигиены для профессиональной прачечной для экологичного и прибыльного цикла обслуживания.',
      },
      'equipment': {
        'title': 'Скачать каталог оборудования',
        'body': 'Стабильность и уверенность в надежности гигиенического оборудования воплощена в нашем оборудовании, проверенном в десятках самых разнообразных проектах комплексного гигиеничского сопровождения предприятий.',
      },
      'all': {
        'title': 'Найти каталоги и документацию для продукта',
        'body': 'Найдите и скачайте все публикации <span class="red">meratech@</span> с упоминанием данного продукта.',
      },
    },
    'go_to_catalog': 'В каталог',
    'go_to_media_center': 'В медиа-центр',
    'go_to_press_center': 'Пресс-центр',
    'hygiene_1': 'Гигиена',
    'hygiene_2': 'Гигиенические',
    'hygiene_solitions': 'Средства гигиены',
    'in': 'в',
    'input_login': 'Введите логин',
    'input_password': 'Введите пароль',
    'kb': 'кБ',
    'latest_releases': 'Последние релизы',
    'latest_news': 'Последние новости',
    'learn_more': 'Узнать больше',
    'login': 'Войти',
    'logout': 'Выйти',
    'months':{
      'january': 'Январь',
      'february': 'Февраль',
      'march': 'Март',
      'april': 'Апрель',
      'may': 'Май',
      'june': 'Июнь',
      'july': 'Июль',
      'august': 'Август',
      'september': 'Сентябрь',
      'october': 'Октябрь',
      'november': 'Ноябрь',
      'december': 'Декабрь'
    },
    'news': 'Новости',
    'news_archive': 'Все новости',
    'no': 'нет',
    'no_data': 'Нет данных',
    'articles_archive': 'Все статьи',
    'no-translation': 'Извините, но этот продукт еще не переведен на этот язык',
    'nothing-found': 'Вы уверены, что ввели все верно?',
    'nothing-found-alter': 'Ничего не найдено',
    'nothing-exist': "Кажется, у нас пока нет таких страниц",
    'nothing-exist-news': "Кажется, у нас пока нет таких новостей",
    'nothing-exist-industry': "Этот раздел пока еще в разработке",
    'nothing-exist-articles': "Кажется, у нас пока нет таких статей",
    'related-materials': 'Cвязанные материалы',
    'page_not_found': {
      'title': 'Досада, кажется мы почистили эту страницу',
      'body': 'Вы можете начать с <a href="/">главной страницы</a> или <a href="/contacts">связаться с нами</a> и рассказать об этом'
    },
    'press': 'Пресс',
    'press_center': 'Пресс-центр',
    'privacy_policy': 'Политика безопасности',
    'professional_1': 'Профессиональное',
    'professional_2': 'Профессиональные',
    'professional_equipment': 'Профессиональное оборудование',
    'results': 'Результаты',
    'search': 'Искать',
    'sign_up': 'Подписаться',
    'slogan':'Профессиональные гигиенические решения',
    'solutions': 'решения',
    'stay_updated': 'Подписаться на новости',
    'stay_updated_text': 'Получайте последние новости от meratech первыми.',
    'terms_of_use': 'Правила использования',
    'to': 'до',
    'footer': {
      'title':'Профессиональные гигиенические решения',
      'body':'На протяжении долгого времени мы разрабатываем и производим самые современные моющие и дезинфицирующие средства для предприятий сельского хозяйства, мясоптицеперерабатывающих и молочных заводов, а также предприятий пивобезалкогольной промышленности.'
    }
  },
  'contacts':{
    'additive': 'доб.',
    'types':{
      'storage': 'Склады',
      'office': 'Офисы'
    }
  },
  'industries':{
    'brewery': 'Пиво и напитки',
    'cow_farms': 'Молочные фермы',
    'laundry': 'Прачечная',
    'milk_industry': 'Молочные предприятия',
    'meat_processing': 'Мясопереработка',
    'pig_farms': 'Свиноводство',
    'poultry': 'Птицефабрики',
    'fish': 'Рыбная промышленность'
  },
  'equipment': {
    'category': {
      'low_pressure_systems':'Системы мойки низкого давления',
      'high_pressure_systems':'Системы мойки высокого давления',
      'lubrication':'Системы смазки конвейеров',
      'co2_systems':'Установки CO<sub>2</sub>',
      'dosing_systems':'Системы дозации',
    },
    'categories': {
      'mobile': 'Мобильное оборудование',
      'stationary': 'Стационарное оборудование',
      'accessory': 'Аксессуары'
    },
    'subcategories': {
      'pumps_module_single': 'Одномодульные насосные станции напольного крепления',
      'posts_onwall': 'Настенные моечные посты',
      'pumps_module_multi': 'Мультимодульные насосные станции напольного крепления',
      'pumps_onwall': 'Насосные станции настенного крепления',
      'inertia_drums': 'Инерционные барабаны',
      'station': 'Мобильные моечные станции',
      'post': 'Мобильные моечные посты',
      'other': '',
      'description': {
        'pumps_module_single': '',
        'posts_onwall': 'Настенные моечные посты <span class="red">meratech&reg;</span> позволяют эффективно осуществлять гигиеническую мойку и дезинфекцию внутрицеховых площадей, открытых поверхностей и оборудования в производственных помещениях пищевых производств. Устанавливаются на стену или любую ровную вертикальную поверхность. Подключаются к системе подачи воды под давлением от <b>15</b> до <b>25</b> Бар. Позволяют оператору работать в трех режимах: ополаскивание струей воды под давлением, нанесение моющего раствора в виде пены, дезинфекция путем распыления дезинфицирующего раствора. В зависимости от выбранной длинны шланга позволяют обрабатывать поверхности на площади от <b>1250</b> до <b>1950</b> м².',
        'pumps_module_multi': '',
        'pumps_onwall': '',
        'inertia_drums': '',
        'station': '',
        'post': '',
        'other': '',
      },
    },
    'advantages': 'Преимущества',
    'articles': 'Артикулы',
    'complectation': 'Комплектация',
    'options': 'Опции',
    'specs': 'Технические характеристики',
    'title':'Оборудование профессионалов',
    'description':'Надежное и производительное оборудование от партнеров-лидеров рынка, которое станeт незаменимым помощником в каждодневной борьбе за качество продукции и благополучие любого пищевого производства.',
    'button_project':'Отправить запрос',
    'title_project':'Проектирование систем<br> автоматической мойки',
    'description_project':'Имея колоссальный опыт по разработке и внедрению комплексных гигиенических решений на предприятиях пищевой промышленности, специалисты <span class="red">meratech&reg;</span> помогут с полным сопровождением вашего индивидуального проекта – от разработки до ввода системы в эксплуатацию.',
    'description_equipment':'Чтобы получить профессиональную консультацию инженероного отдела и каталог этой категории свяжитесь с нашим центральным офисом.',
  },
  'press_center':{
    'looking_for': 'ищу новости',
    'company': 'Компания',
    'products': 'Продукты',
    'clients': 'Клиентам',
    'events': 'События',
    'company_': 'О компании',
    'products_': 'О продуктах',
    'clients_': 'Для клиентов',
    'events_': 'О событиях',
  },
  'media_center':{
    'media_center_title': 'Медиа и документы',
    'media': 'медиа',
    'search_for': 'поиск по',
    'media_center_slogan': 'Найдите все документы, которые вам нужны',
    'media_center_instruction': 'Введите название продукта в строку поиска выше и нажмите Enter.',
    'leaflet': 'Информационный лифлет',
    'registration_ru': 'Свидетельство о регистрации ЕврАзЭС',
    'registration_en': 'Свидетельство о регистрации',
    'leaflet_': 'Паспорт безопасности',
    'registration_': 'Инструкция по использованию',
    'types':{
      'catalog' : 'Каталог',
      'article' : 'Статья',
      'brochure' : 'Буклет',
      'publication' : 'Публикация',
      'video' : 'Видео'
    },
  },
  'catalog':{
    'types':{
      'opc_cleaning': 'Внешняя мойка',
      'cip_cleaning': 'Внутренняя мойка',
      'agriculture': 'Животноводство',
      'disinfectant': 'Дезинфекция',
      'additive': 'Добавки',
      'lubricant': 'Смазки',
      'laundry': 'Прачечная',
      'personal': 'Гигиена персонала',
      'other': 'Прочие продукты',

      'before_milking' : 'Обработка сосков вымени перед доением',
      'after_milking' : 'Обработка сосков вымени после доения',
      'cleaning_milking_equipment' : 'Мойка и дезинфекция доильного оборудования',
      'cleaning_warehouse' : 'Комплексная дезинфекция помещений',
      'hoof_hygiene' : 'Обработка копыт',

      'poultry_warehouse_cleaning' : 'Мойка корпусов содержания птицы',
      'poultry_drink' : 'Гигиена систем поения',
      'poultry_water' : 'Обеззараживание питьевой воды',
      'poultry_warehouse_disinfection' : 'Дезинфекция корпусов содержания птицы',
      'poultry_egg' : 'Очистка и дезинфекция инкубационного яйца',
      'poultry_incubatory' : 'Гигиена инкубационных шкафов',
      'poultry_meat' : 'Антимикробная обработка тушек птицы',

      'pigs_warehouse_disinfection' : 'Комплексная дезинфекция помещений',
      'pigs_warehouse_cleaning' : 'Мойка помещений содержания животных',
      'pigs_outer_equipment' : 'Наружная мойка технологического оборудования',
      'pigs_drink' : 'Гигиена систем поения',
      'pigs_water' : 'Обеззараживание питьевой воды',
      'pigs' : 'Дезинфекция автотранспорта',

      'meat_opc' : 'Внешняя мойка (OPC — Open Plant Cleaning)',
      'meat_processing_house' : 'Мойка помещений цехов переработки',
      'meat_equipment_disinfection' : 'Дезинфекция оборудования',
      'meat_steel' : 'Полировка и уход за стальными поверхностями',
      'meat_floor' : 'Чистка напольных покрытий',
      'meat_complicated_dirt' : 'Чистка и удаление сложных загрязнений',
      'meat_auto' : 'Мойка автотранспорта',
      'meat_other' : 'Специальные средства',
      'meat_thermo' : 'Мойка термокамер и варочных шкафов',
      'meat_cip' : 'Внутренняя мойка (CIP – Cleaning In Place)',
      'meat_water' : 'Замачивание съемных деталей оборудования',
      'meat_plastic' : 'Мойка пластиковой тары',
      'meat_personal' : 'Гигиена персонала',

      'brewery_cip' : 'Внутренняя мойка (CIP – Cleaning In Place)',
      'brewery_opc' : 'Внешняя мойка (ОРС - Open Plant Cleaning)',
      'brewery_additive' : 'Добавки в бутылкомоечную машину',
      'brewery_lubrication' : 'Смазка конвейерных лент',
      'brewery_keg' : 'Мойка кегов',
      'brewery_pvc' : 'Мойка поликарбонатных бутылей',
      'brewery_steel' : 'Полировка и уход за стальными поверхностями',
      'brewery_floor' : 'Чистка напольных покрытий',
      'brewery_plastic' : 'Мойка пластиковой тары',
      'brewery_tunnel' : 'Туннельные пастеризаторы',
      'brewery_personal' : 'Гигиена персонала',
    },
    'documentation':{
      'reg': 'Свидетельство о регистрации',
      'leaflet': 'Лифлет',
      'pds': 'Инструкция',
      'sds': 'Паспорт безопасности продукта'
    },
    'phrases':{
      'R1' : 'В сухом виде взрывоопасно',
      'R2' : 'Риск взрыва при встряхивании, трении, воздействии огня и других действиях, вызывающих возгорание',
      'R3' : 'Чрезвычайно взрывоопасно при встряхивании, трении, контакте с огнём и других действиях, вызывающих возгорание',
      'R4' : 'Образует чрезвычайно взрывоопасные соединения с металлами',
      'R5' : 'Нагревание может вызвать взрыв',
      'R6' : 'Взрывоопасно как при контакте с воздухом, так и в его отсутствии',
      'R7' : 'Может вызвать возгорание',
      'R8' : 'Контакт с горючими веществами может вызвать возгорание',
      'R9' : 'Взрывоопасно при смешивании с горючими веществами',
      'R10' : 'Огнеопасно',
      'R11' : 'Очень огнеопасно',
      'R12' : 'Чрезвычайно огнеопасно',
      'R14' : 'Вступает в бурную реакцию с водой',
      'R15' : 'При контакте с водой образует чрезвычайно горючие газы',
      'R16' : 'Взрывоопасно при смешивании с окислителями',
      'R17' : 'Самовоспламеняется на воздухе',
      'R18' : 'При использовании может образовывать взрывоопасные/огнеопасные смеси паров с воздухом',
      'R19' : 'Может образовывать взрывоопасные перекиси',
      'R20' : 'Опасно при вдыхании',
      'R21' : 'Опасно при попадании на кожу',
      'R22' : 'Опасно при проглатывании',
      'R23' : 'Токсично при вдыхании',
      'R24' : 'Токсично при попадании на кожу',
      'R25' : 'Токсично при проглатывании',
      'R26' : 'Очень токсично при вдыхании',
      'R27' : 'Очень токсично при попадании на кожу',
      'R28' : 'Очень токсично при проглатыавании',
      'R29' : 'При контакте с водой выделяет токсичный газ',
      'R30' : 'Может стать очень огнеопасным в процессе использования',
      'R31' : 'При контакте с кислотами выделяет токсичный газ',
      'R32' : 'При контакте с кислотами выделяет очень токсичный газ',
      'R33' : 'Опасность кумулятивных эффектов',
      'R34' : 'Вызывает ожоги',
      'R35' : 'Вызывает сильные ожоги',
      'R36' : 'Вызывает раздражение глаз',
      'R37' : 'Вызывает раздражение органов дыхания',
      'R38' : 'Вызывает раздражение кожи',
      'R39' : 'Риск очень серьёзных необратимых эффектов',
      'R40' : 'Имеются некоторые доказательства канцерогенных эффектов',
      'R41' : 'Риск серьёзного повреждения глаз',
      'R42' : 'Может вызвать сенсибилизацию при вдыхании',
      'R43' : 'Может вызвать сенсибилизацию при попадании на кожу',
      'R44' : 'Риск взрыва при нагревании в ограниченном пространстве',
      'R45' : 'Может вызвать рак',
      'R46' : 'Может вызвать наследственные генетические нарушения',
      'R48' : 'Опасность серьёзного вреда здоровью при длительном воздействии',
      'R49' : 'Может вызвать рак при вдыхании',
      'R50' : 'Очень токсично для водных организмов',
      'R51' : 'Токсично для водных организмов',
      'R52' : 'Опасно для водных организмов',
      'R53' : 'Может оказывать продолжительное неблагоприятное воздействие на водную среду',
      'R54' : 'Токсично для растений',
      'R55' : 'Токсично для животных',
      'R56' : 'Токсично для почвенных организмов',
      'R57' : 'Токсично для пчел',
      'R58' : 'Может оказывать продолжительное неблагоприятное воздействие на окружающую среду',
      'R59' : 'Опасно для озонового слоя',
      'R60' : 'Может нарушить плодовитость',
      'R61' : 'Может причинить вред нерожденному ребёнку',
      'R62' : 'Возможен риск нарушения способности к зачатию',
      'R63' : 'Возможен риск вреда для нерожденного ребёнка',
      'R64' : 'Может причинить вред младенцу при грудном вскармливании',
      'R65' : 'Опасно: может причинить вред лёгким при проглатывании',
      'R66' : 'Повторяющееся воздействие может вызвать сухость и растрескивание кожи',
      'R67' : 'Пары могут вызвать сонливость и головокружение',
      'R68' : 'Возможен риск необратимых эффектов',
      'R14/15' : 'Вступает в сильную реакцию с водой с выделением очень огнеопасных газов',
      'R15/29' : 'При контакте с водой выделяет токсичный, чрезвычайно огнеопасный газ',
      'R20/21' : 'Опасно при вдыхании и попадании на кожу',
      'R20/21/22' : 'Опасно при вдыхании, попадании на кожу и проглатывании',
      'R20/22' : 'Опасно при вдыхании и проглатывании',
      'R21/22' : 'Опасно при попадании на кожу и проглатывании',
      'R23/24' : 'Токсично при вдыхании и попадании на кожу',
      'R23/24/25' : 'Токсично при вдыхании, попадании на кожу и проглатывании',
      'R23/25' : 'Токсично при вдыхании и проглатывании',
      'R24/25' : 'Токсично при попадании на кожу и проглатывании',
      'R26/27' : 'Очень токсично при вдыхании и попадании на кожу',
      'R26/27/28' : 'Очень токсично при вдыхании, попадании на кожу и проглатывании',
      'R26/28' : 'Очень токсично при вдыхании и проглатывании',
      'R27/28' : 'Очень токсично при попадании на кожу и проглатывании',
      'R36/37' : 'Раздражает глаза и органы дыхания',
      'R36/37/38' : 'Раздражает глаза, органы дыхания и кожу',
      'R36/38' : 'Раздражает глаза и кожу',
      'R37/38' : 'Раздражает органы дыхания и кожу',
      'R37/39' : 'Может привести к серьезным повреждениям органов дыхания',
      'R39/23' : 'Токсично: опасность очень серьёзных необратимых последствий при вдыхании',
      'R39/23/24' : 'Токсично: опасность очень серьёзных необратимых последствий при вдыхании и попадании на кожу',
      'R39/23/24/25' : 'Токсично: опасность очень серьёзных необратимых последствий при вдыхании, попадании на кожу и проглатывании',
      'R39/23/25' : 'Токсично: опасность очень серьёзных необратимых последствий при вдыхании и проглатывании',
      'R39/24' : 'Токсично: опасность очень серьёзных необратимых последствий при попадании на кожу',
      'R39/24/25' : 'Токсично: опасность очень серьёзных необратимых последствий при попадании на кожу и проглатывании',
      'R39/25' : 'Токсично: опасность очень серьёзных необратимых последствий при проглатывании',
      'R39/26' : 'Очень токсично: опасность очень серьёзных необратимых последствий при вдыхании',
      'R39/26/27' : 'Очень токсично: опасность очень серьёзных необратимых последствий при вдыхании и попадании на кожу',
      'R39/26/27/28' : 'Очень токсично: опасность очень серьёзных необратимых последствий при вдыхании, попадании на кожу и проглатывании',
      'R39/26/28' : 'Очень токсично: опасность очень серьёзных необратимых последствий при вдыхании и проглатывании',
      'R39/27' : 'Очень токсично: опасность очень серьёзных необратимых последствий при попадании на кожу',
      'R39/27/28' : 'Очень токсично: опасность очень серьёзных необратимых последствий при попадании на кожу и проглатывании',
      'R39/28' : 'Очень токсично: опасность очень серьёзных необратимых последствий при проглатывании',
      'R42/43' : 'Может вызвать неприятные ощущения при вдыхании и попадании на кожу',
      'R48/20' : 'Вредно: может нанести серьёзный вред здоровью при длительном вдыхании',
      'R48/20/21' : 'Вредно: может нанести серьёзный вред здоровью при длительном вдыхании и контакте с кожей',
      'R48/20/21/22' : 'Вредно: может нанести серьёзный вред здоровью при длительном вдыхании, попадании на кожу и проглатывании',
      'R48/20/22' : 'Вредно: может нанести серьёзный вред здоровью при длительном вдыхании и проглатывании',
      'R48/21' : 'Вредно: может нанести серьёзный вред здоровью при длительном контакте с кожей',
      'R48/21/22' : 'Вредно: может нанести серьёзный вред здоровью при длительном контакте с кожей и проглатывании',
      'R48/22' : 'Вредно: может нанести серьёзный вред здоровью при проглатывании',
      'R48/23' : 'Токсично: опасность нанесения серьёзного вреда здоровью при длительном вдыхании',
      'R48/23/24' : 'Токсично: опасность нанесения серьёзного вреда здоровью при длительном вдыхании и контакте с кожей',
      'R48/23/24/25' : 'Токсично: опасность нанесения серьёзного вреда здоровью при длительном вдыхании, контакте с кожей или проглатывании',
      'R48/23/25' : 'Токсично: опасность нанесения серьёзного вреда здоровью при длительном вдыхании и проглатывании',
      'R48/24' : 'Токсично: опасность нанесения серьёзного вреда здоровью при длительном длительном контакте с кожей',
      'R48/24/25' : 'Токсично: опасность нанесения серьёзного вреда здоровью при длительном длительном контакте с кожей или проглатывании',
      'R48/25' : 'Токсично: опасность нанесения серьёзного вреда здоровью при длительном воздействии при проглатыавании',
      'R50/53' : 'Очень токсично для водных организмов, может вызывать продолжительные неблагоприятные изменения в водной среде',
      'R51/53' : 'Токсично для водных организмов, может вызывать продолжительные неблагоприятные изменения в водной среде',
      'R52/53' : 'Опасно для водных организмов, может вызывать продолжительные неблагоприятные изменения в водной среде',
      'R68/20' : 'Вредно: возможны необратимые последствия при вдыхании',
      'R68/20/21' : 'Вредно: возможны необратимые последствия при вдыхании и попадании на кожу',
      'R68/20/21/22' : 'Вредно: возможны необратимые последствия при вдыхании, попадании на кожу и проглатывании',
      'R68/20/22' : 'Вредно: возможны необратимые последствия при вдыхании и проглатывании',
      'R68/21' : 'Вредно: возможны необратимые последствия при попадании на кожу',
      'R68/21/22' : 'Вредно: возможны необратимые последствия при попадании на кожу и проглатывании',
      'R68/22':  'Вредно: возможны необратимые последствия при проглатывании',

      'S1' : 'Хранить под замком',
      'S2' : 'Хранить в местах, недоступных для детей',
      'S3' : 'Хранить в прохладном месте',
      'S4' : 'Хранить вдали от жилых помещений',
      'S5' : 'Хранить содержимое под … (соответствующую жидкость указывает производитель)',
      'S6' : 'Хранить под … (инертный газ указывает производитель)',
      'S7' : 'Держать контейнер плотно закрытым',
      'S8' : 'Хранить контейнер в сухом месте',
      'S9' : 'Хранить контейнер в хорошо проветриваемом помещении',
      'S12' : 'Не запечатывать контейнер',
      'S13' : 'Хранить вдали от продуктов питания, напитков и корма для животных',
      'S14' : 'Хранить вдали от … (несовместимые вещества указывает производитель)',
      'S15' : 'Хранить вдали от источников тепла',
      'S16' : 'Хранить вдали от источников возгорания — Не курить',
      'S17' : 'Хранить вдали от горючих материалов',
      'S18' : 'Обращаться и вскрывать с осторожностью',
      'S20' : 'Не принимать пищу и напитки при работе с материалом',
      'S21' : 'Не курить при работе с материалом',
      'S22' : 'Не вдыхать пыль',
      'S23' : 'Не вдыхать газ/пары/аэрозоли (нужное указывает производитель)',
      'S24' : 'Избегать попадания на кожу',
      'S25' : 'Избегать попадания в глаза',
      'S26' : 'В случае попадания в глаза немедленно промыть глаза большим количеством воды и обратиться за медицинской помощью',
      'S27' : 'Немедленно снять всю загрязненную одежду',
      'S28' : 'При попадании на кожу немедленно промыть большим количеством … (вещество указывает производитель)',
      'S29' : 'Не сливать в канализацию',
      'S30' : 'Никогда не приливать воду к данному материалу',
      'S33' : 'Примите меры предосторожности во избежание статических разрядов',
      'S35' : 'Материал и тара должны утилизироваться безопасным образом',
      'S36' : 'Наденьте соответствующую защитную одежду',
      'S37' : 'Наденьте соответствующие перчатки',
      'S38' : 'В условиях недостаточной вентиляции работайте в соответствующей респираторной экипировке',
      'S39' : 'Используйте средства защиты глаз/лица',
      'S40' : 'Для обработки пола и других предметов, загрязненных данным материалом, используйте … (вещество указывает производитель)',
      'S41' : 'В случае пожара и/или взрыва не вдыхать дым',
      'S42' : 'При образовании паров/аэрозоля использовать соответствующие респираторы (тип респиратора указывает производитель)',
      'S43' : 'В случае пожара использовать … (указать средства пожаротушения. Если вода увеличивает риск, добавить «Не тушить водой»)',
      'S45' : 'В случае аварии или при плохом самочувствии немедленно обратиться за медицинской помощью (по возможности предъявить этикетку материала)',
      'S46' : 'При проглатывании немедленно обратиться за медицинской помощью и предъявить упаковку или этикетку материала',
      'S47' : 'Хранить при температурах, не превышающих … °C (указывает производитель)',
      'S48' : 'Хранить увлажненным … (вещество указывает производитель)',
      'S49' : 'Хранить только в оригинальной упаковке',
      'S50' : 'Не смешивать с … (указывает производитель)',
      'S51' : 'Работать только в хорошо проветриваемых помещениях',
      'S52' : 'Не рекомендуется использование внутри помещения на больших площадях',
      'S53' : 'Избегать контакта — перед использованием получить специальные инструкции',
      'S56' : 'Отработанный материал и тару сдавать на специальный пункт сбора опасных отходов',
      'S57' : 'Во избежание загрязнения окружающей среды использовать соответствующую тару',
      'S59' : 'Обратитесь к производителю/поставщику за информацией по переработке/утилизации материала',
      'S60' : 'Данный материал и его тару следует утилизировать как опасные отходы',
      'S61' : 'Не допускать попадания в окружающую среду. Смотрите специальные инструкции/паспорт безопасности материала',
      'S62' : 'В случае проглатывания не вызывать рвоту немедленно обратиться за медицинской помощью и показать упаковку или этикетку материала',
      'S63' : 'В случае вдыхания вывести пострадавшего на свежий воздух и обеспечить покой',
      'S64' : 'При проглатывании промыть рот водой (только если пострадавший в сознании)',
      'S1/2' : 'Хранить под замком в местах, недоступных для детей',
      'S3/7' : 'Хранить в плотно закрытом контейнере в прохладном месте',
      'S3/7/9' : 'Хранить в прохладном и хорошо проветриваемом месте в плотно закрытой таре',
      'S3/9/14' : 'Хранить в прохладном, хорошо проветриваемом месте вдали от … (несовместимые материалы указывает производитель)',
      'S3/9/14/49' : 'Хранить только в оригинальной таре в прохладном, хорошо проветриваемом месте вдали от … (несовместимые материалы указывает производитель)',
      'S3/9/49' : 'Хранить только в оригинальной таре в прохладном, хорошо проветриваемом месте',
      'S3/14' : 'Хранить в прохладном месте вдали от … (несовместимые материалы указывает производитель)',
      'S7/8' : 'Хранить контейнер плотно закрытым и сухим',
      'S7/9' : 'Хранить контейнер плотно закрытым и в хорошо проветриваемом помещении',
      'S7/47' : 'Хранить контейнер плотно закрытым при температуре не выше … °C (температуру указывает производитель)',
      'S20/21' : 'Во время работы не принимать пищу и напитки, не курить',
      'S24/25' : 'Избегать попадания на кожу и в глаза',
      'S27/28' : 'При попадании на кожу немедленно снять всю загрязненную одежду и промыть пострадавшее место большим количеством … (вещество указывает производитель)',
      'S29/35' : 'Не сливать в канализацию; утилизировать материал и тару безопасным способом',
      'S29/56' : 'Не сливать в канализацию, для утилизации сдавать материал и тару на специальный пункт сбора опасных отходов',
      'S36/37' : 'Надеть соответствующую защитную одежду и перчатки',
      'S36/37/39' : 'Надеть соответствующую защитную одежду, перчатки и средства защиты глаз/лица',
      'S36/39' : 'Надеть соответствующую защитную одежду и средства защиты глаз/лица',
      'S37/39' : 'Надеть соответствующие перчатки и средства защиты глаз/лица',
      'S47/49' : 'Хранить только в оригинальной таре при температуре, не превышающей температуру хранения',
    },
    'mark' : {
      'mark_c' : 'Corrosive<br>Едкое',
      'mark_o' : 'Oxidizing<br>Окислитель',
      'mark_n' : 'Dangerous for the environment<br>Опасно для окружающей среды',
      'mark_t' : 'Toxic<br>Токсичное',
      'mark_x' : 'Harmful<br>Вредно',
      'mark_f' : 'Flammable<br>Огнеопасно'
    },
    'advantages': 'Преимущества',
    'equipment_slogan': 'Все оборудование',
    'equipment_text': 'Если вам нужно комплексное решение, включающее в себя установку оборудования, ознакомьтесь с ним в этом разделе, а потом свяжитесь с нашими менеджерами для консультации по вашему проекту.',
    'scope': 'Сфера применения',
    'foaming': 'Пенообразование',
    'foaming_alt': 'Пенный',
    'not_foaming': 'Беспенный',
    'ph_scale': 'Кислотность (рН)',
    'acidic': 'Кислотный',
    'neutral': 'Нейтральный',
    'alkalic': 'Щелочной',
    'usage': 'Применение',
    'date_at': 'от',
    'evrazes': 'ЕврАзЭС',
    'about_product': 'О продукте',
    'marking': 'Маркировка',
    'safety': 'Безопасность',
    'risk_phrases': 'Фразы риска',
    'safety_phrases': 'Фразы опасности',
    'properties': 'Свойства',
    'consistency': 'Консистенция',
    'color': 'Цвет',
    'density': 'Плотность',
    'temperature': 'Температура хранения',
    'precautions': 'Меры предосторожности',
    'breath_safety': 'Защита органов дыхания',
    'hands_safety': 'Защита рук',
    'eyes_safety': 'Защита глаз',
    'storage_info': 'Хранение',
    'storage': 'Способы хранения',
    'environment': 'Окружающая среда',
    'package': 'Упаковка',
    'kg': 'кг',
    'fluid': 'Жидкость',
    'gel': 'Гель',
    'powder': 'Порошок',
    'emulsion': 'Эмульсия',
    'tablets': 'Таблетки',
    'related_equipment': 'Связанное оборудование'
  }
};